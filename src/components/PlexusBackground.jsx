...
const PlexusBackground = () => {
  ...
  const setup = (p5, canvasParentRef) => {
    ...
    p5.mousePressed = () => {
      particles.forEach(particle => {
        if (particle.isClicked(p5.mouseX, p5.mouseY)) {
          createNewParticles(p5, particle.pos.x, particle.pos.y);
        }
      });
    };
  }
  ...
  const createNewParticles = (p5, x, y) => {
    const newParticlesCount = 5; // Number of new particles to create
  ...
  class Particle {
    ...
    checkParticles(particles) {
      particles.forEach(particle => {
        ...
        if (d < 150) {
          this.p5.stroke('rgba(255, 0, 0, 0.1)');
          this.p5.line(this.pos.x, this.pos.y, particle.pos.x, particle.pos.y);
        }
      });
    }
    ...
    isClicked(mouseX, mouseY) {
      const d = this.p5.dist(this.pos.x, this.pos.y, mouseX, mouseY);
      return d < this.size; // Check if the mouse is within the size of the particle
    }
  }
